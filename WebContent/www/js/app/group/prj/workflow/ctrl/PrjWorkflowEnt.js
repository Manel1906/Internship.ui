define([
	'group/prj/workflow/ctrl/PrjWorkflowEntTab',
	'group/prj/workflow/ctrl/PrjWorkflowEntTabChartFlow'
	],
	function(	
			{
				PrjWorkflowEntTabContent, 
				PrjWorkflowEntTabStat, 
				PrjWorkflowEntTabMember
			},
			PrjWorkflowEntTabChartFlow
	){

	var PrjWorkflowEnt 	= function (grpName, header, content, footer) {
		...
		const do_lc_init_ctrl_module = () => {
			let pr_LST_MODULE = {
					EntMember 		: PrjWorkflowEntTabMember	,  
					EntStat 		: PrjWorkflowEntTabStat		,
					EntContent 		: PrjWorkflowEntTabContent	, 
					EntChartFlow	: PrjWorkflowEntTabChartFlow,
			}
		...
		this.do_lc_show = function(id, code, divContent, typ00, isPopup = false){
			try{
				...
				console.log(e); 
				//do_gl_send_exception(App.path.BASE_URL_API_PRIV, App.data["HttpSecuHeader"], App.network, "prj.project", "PrjWorkflowEnt", "do_lc_show", e.toString()) ;
			}
		};
...
//				$("#chart_container").addClass("div_disabled")
		...
		const do_lc_show_prj = prj => {
			...
			$(pr_DIV_CONTENT)	.html(tmplCtrl.req_lc_compile_tmpl(tmplName.PRJ_WORKFLOW_ENT, {
				prj,
				isFavorite,
			}));

			pr_project.EntStat		.do_lc_req_lstStats		(prj);
			pr_project.EntContent	.do_lc_show_prj_content	(prj, null);
			
			var scrollToTop = false;

			pr_project.EntMember					.do_lc_get_members			(prj, scrollToTop);
			pr_project.EntChartFlow					.do_lc_show					(prj);
