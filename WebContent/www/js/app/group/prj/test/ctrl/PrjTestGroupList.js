	...
	const PrjTestGroupList = function (grpName, header, content, footer) {
		...
		this.do_lc_show = function(pGroup, pId, pCode){               
			try{
				...
				console.log(e); //do_gl_send_exception(App.path.BASE_URL_API_PRIV, App.data["HttpSecuHeader"], App.network, "prj.project", "PrjTestGroupList", "do_lc_show", e.toString()) ;
			}
		};
		...
		const do_lc_init_element = obj => {
			...
			App.SummerNoteController.do_lc_show("#div_create_prj_test_group");//text editor
