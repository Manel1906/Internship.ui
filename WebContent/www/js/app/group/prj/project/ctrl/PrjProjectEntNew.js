define([
	...
	const PrjProjectEntNew     = function (grpName, header, content, footer) {
		...
		const do_lc_init_element = obj => {
			const avatarFile = do_gl_init_fileDropzone($("#div_prj_avatar"), {
...
//					autoProcessQueue :false; //for not auto send file
			...
			do_lc_bind_event_dtInput(obj);
		}

		const do_lc_bind_event_dtInput = (obj) => {
		...
		const do_lc_req_autocomplete = (obj) => {
			...
			let options 	= {
			    ...
			    hintSvParams	: {wAvatar:true, nbline:20, typ01s: typ01Str, stats:1, entId01: obj.grp}, 
		...
		const do_lc_req_autocomplete_group = () => {
			...
			let optionsG = {
			    dataService 	: [pr_SERVICE_GROUP_CLASS, pr_SV_GROUP_SEARCH],
			    dataRes 		: ["name"], 
			    svParams		: {wAvatar:true, nbLine:20, typ01s: 300},
...
//					await do_gl_send_data_with_dropzone(); // if not auto send file, add line and async to function on event
			...
			do_lc_req_autocomplete		(obj);
			do_lc_req_autocomplete_group(obj);
		};
	}
