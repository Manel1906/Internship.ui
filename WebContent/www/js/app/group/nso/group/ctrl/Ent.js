define([
		'jquery',
		'text!group/nso/group/tmpl/Ent.html',
		
		'group/nso/group/ctrl/EntBtn', 
		'group/nso/group/ctrl/EntHeader',
		'group/nso/group/ctrl/EntTabMember' 
		...
		var CtrlEnt     = function (grpName, header, content, footer) {
			...
			this.do_lc_init		= function(){
				...
				if (!App.controller[pr_grpName].EntTabMem	)  
					 App.controller[pr_grpName].EntTabMem		= new CtrlEntTabMem			(null, "#div_Ent_Tab_Member", null, pr_grpName);
				
				App.controller[pr_grpName].EntBtn		.do_lc_init();
				App.controller[pr_grpName].EntHeader	.do_lc_init();
				App.controller[pr_grpName].EntTabMem	.do_lc_init();
				//--------------------------------------------------------------------
				...
				pr_ctr_EntTabMem 		= App.controller[pr_grpName].EntTabMem;
			...
			this.do_lc_show		= function(obj, mode){	
				try{
					...
					pr_ctr_EntTabMem	.do_lc_show(pr_obj, pr_mode);
...
//				fSucces.push(req_gl_funct(null	, do_refresh_list				, []));
			...
			var do_refresh_list = function(sharedJSon, obj) {
				if(sharedJSon[App['const'].SV_CODE] == App['const'].SV_CODE_API_YES) {
//					pr_ctr_List.do_lc_show_byTypeStat(obj.typ01, obj.stat01);
				}
			}
