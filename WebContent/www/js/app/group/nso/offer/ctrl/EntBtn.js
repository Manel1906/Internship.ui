define([
	'jquery',
	'text!group/nso/offer/tmpl/Ent_Btn.html',
	'text!group/nso/offer/tmpl/Ent_Btn_Sub.html',
	'text!group/nso/offer/tmpl/Ent_Btn_Lang.html'
],
	...
	function ($,
		...
		var CtrlEntBtn = function (grpName, header, content, footer) {
			var self = this;
			...
			var POST_STAT_PRIVATE = 4;
			var POST_STAT_PUBLIC = 4;
			...
			this.do_lc_show = function (obj, mode) {
				...
				try {
					...
					do_gl_exception_send(App.path.BASE_URL_API_PRIV, "nso.offer", "CtrlEntBtn", "do_lc_show", e.toString());
				}
			}
			...
			var do_bind_event_btn_validate = function (obj, mode) {
				...
				$(pr_btn_Validate).click(function () {
					pr_ctr_Ent.do_update_offer_status(obj, POST_STAT_ACCEPTED);
				});
			}
			...
			var do_bind_event_btn_deny = function (obj, mode) {
				...
				$(pr_btn_Deny).click(function () {
					pr_ctr_Ent.do_update_offer_status(obj, POST_STAT_DENIED);
				});
			}
			...
			var doCloneLang = function (obj) {
				...
				var objId = obj.id;
				window.open("view_nso_offer_stuwork.html?id=" + objId + "&mode=" + modeTransl + "&lang=" + lang, "_blank");
			}
			...
			var do_bind_event_btn_see = function (obj, mode) {
				...
				$(pr_btn_See).click(function () {
					...
					if (mode == App['const'].MODE_TRANSL) {
						window.open("/view_offer.html?forManager=true&id=" + offerId + "&lang=" + lang, "_blank");
					} else {
						window.open("/view_offer.html?forManager=true&id=" + offerId, "_blank");
					}
				});
			}
