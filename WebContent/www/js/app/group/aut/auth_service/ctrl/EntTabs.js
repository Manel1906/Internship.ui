define([
        'jquery',
        'text!group/aut/auth_service/tmpl/Ent_Tabs.html',

		'group/aut/auth_service/ctrl/EntTabInfo' ,
		'group/aut/auth_service/ctrl/EntTabDoc' 
        ],
        function($, 
        		Tmpl_Ent_Tabs ,     
	...
	var CtrlEntTabs     = function (grpName, header, content, footer) {
		...
		this.do_lc_init		= function(){
			...
			if (!App.controller[pr_grpName].EntTabInfo	)  
				 App.controller[pr_grpName].EntTabInfo	= new CtrlEntTabInfo		(null, "#div_Ent_Tab_Info", null, pr_grpName);
			...
			pr_ctr_EntTab_Info		= App.controller[pr_grpName].EntTabInfo;
...
//				do_gl_calculateScrollBody(pr_divContent + " .custom-scroll-tab", 43.6);
				...
				pr_ctr_EntTab_Info	.do_lc_show(obj, mode);
				pr_ctr_EntTab_Doc	.do_lc_show(obj, mode);
				
				do_bind_event(obj, mode);
			}catch(e) {			
				console.log(e);
				do_gl_exception_send(App.path.BASE_URL_API_PRIV,  "aut.auth_service", "EntTabs", "do_lc_show", e.toString()) ;
			}
		};
