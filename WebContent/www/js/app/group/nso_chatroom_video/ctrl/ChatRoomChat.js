define([
	'group/nso_chatroom_video/ctrl/Webcam',
	'group/nso_chatroom_video/ctrl/Recorder',
...
//			initialeValues.currentTyp 		= typChat;
		...
		const do_lc_show_grp_btn_mobile = function(typeChat, group){
			...
			if (typeChat==pr_TYP_CHAT_USER){
				...
				$("#btn_chat_post").hide();
				let userChat = App.data.listUser[group.iduser];
				let userChatSubset = {
				    typ01: group.typ01,
				    avatar: userChat.avatar,
				    login01: userChat.login01
				};
				pr_ctr_Main.do_lc_avatar_moblie(userChatSubset);
				
			}else if(pr_typChat === 2 || group.typ01 ===201){
			...
			try{
				if (lstHasHistory ){
					lstHasHistory.forEach((e) => {
						...
						if (liMsg.find(".li_msg_read_" + e.id).length==0){
							liMsg.append ("<li class='message-item message-read li_msg_read_"+e.id+"'></li>")
						}
						liMsg.find(".li_msg_read_" + e.id).append(tmplCtrl.req_lc_compile_tmpl(tmplName.CHATROOM_TAB_CHAT_CONTENT_MSG_READ	, e));
					})
				}
			}catch(e){
				console.log(e);
