define([
	...
	function(TpyCatBlood_List, 
			TpyCatBlood_List_Content,
			TpyCatBlood_Ent_Content,
			TpyCatBlood_New,
			TpyDropzone_File,
			TpyCatBlood_Ent_Content_Row,
			TpyCatBlood_Ent_Content_Row_add
			) {
	const TpyCatBloodList = function (grpName, header, content, footer) {
		...
		const pr_SV_LIST_TEST		= "SVLstPageTestBlood"; 
		...
		this.do_lc_init		= function(){
			...
			pr_ctr_List 			= App.controller.TpyCatBlood.List
			if(!tmplName) {
				App.template.names[pr_grpName] = {}
				tmplName = App.template.names[pr_grpName]
			}
			
			tmplName.TPY_CAT_Blood_LIST					= "TpyCatBlood_List";
			tmplName.TPY_CAT_Blood_LIST_CONTENT			= "TpyCatBlood_List_Content";
			tmplName.TPY_CAT_Blood_ENT_CONTENT			= "TpyCatBlood_Ent_Content";
			tmplName.TPY_CAT_Blood_NEW  					= "TpyCatBlood_New";
			tmplName.TPY_DROPZONE_FILE						= "TpyDropzone_File";
			tmplName.TPY_CAT_Blood_ENT_CONTENT_ROW		= "TpyCatBlood_Ent_Content_Row";
			tmplName.TPY_CAT_Blood_ENT_CONTENT_ROW_ADD	= "TpyCatBlood_Ent_Content_Row_add";
		}
		...
		const do_lc_load_view = function(){
			tmplCtrl.do_lc_put_tmpl(tmplName.TPY_CAT_Blood_LIST					, TpyCatBlood_List); 
			tmplCtrl.do_lc_put_tmpl(tmplName.TPY_CAT_Blood_LIST_CONTENT			, TpyCatBlood_List_Content); 	
			tmplCtrl.do_lc_put_tmpl(tmplName.TPY_CAT_Blood_ENT_CONTENT			, TpyCatBlood_Ent_Content); 	
			tmplCtrl.do_lc_put_tmpl(tmplName.TPY_CAT_Blood_NEW					, TpyCatBlood_New); 	
		...
		const do_lc_get_list_member = (group) => {
			App.controller.TpyCatBlood.Member.do_lc_show(group);
		};
		...
		const do_lc_bind_event = function(obj){
			...
			$("#btn_add_doc").off("click").on("click", function(){
				...
				App.MsgboxController.do_lc_show({
					title	: $.i18n("tpy_cat_Blood_new_file_title"),
					width	: "500px",
					autoclose	: true,
...
//			if(data.hasError)	return false;
		...
		const do_get_list_ByAjax = function(hardLoad){	
			...
			const ref 				= req_gl_Request_Content_Send_With_Params(pr_SERVICE_CLASS_DYN, pr_SV_LIST_TEST, {typ01s: pr_TYP_TEST_BLOOD, searchKey: pr_SEARCH_KEY, stats : pr_typ, hardLoad, wChild: true});
		...
		const do_lc_show_list_ByAjax_Dyn = function(sharedJson, divList){
			...
			if(isSuccess) {
				...
				if (!lst.length) {
					$(divList).html(tmplCtrl.req_lc_compile_tmpl(tmplName.TPY_CAT_Blood_LIST_CONTENT, {}));
				...
				$(divList).html(tmplCtrl.req_lc_compile_tmpl(tmplName.TPY_CAT_Blood_LIST_CONTENT, { "data": data.lst }));
		...
		this.do_lc_save = function(obj){
			...
			do_lc_new_test(data.data);
		}
		
		//-----------------new group-------------------------------------------------------------------------
		
		const do_lc_new_test = function(group){
			const ref 		= req_gl_Request_Content_Send_With_Params(pr_SERVICE_CLASS, pr_SV_NEW, {obj: group});

			let fSucces		= [];
			fSucces.push(req_gl_funct(null, do_lc_afterCreat_Test, [group]));
		...
		const do_lc_afterCreat_Test = function(sharedJson, group){
			if(can_gl_AjaxSuccess(sharedJson)) {
		...
		const do_lc_show_info_group = (data) => {
			...
			$("#tbody_Blood").html(tmplCtrl.req_lc_compile_tmpl(tmplName.TPY_CAT_Blood_ENT_CONTENT_ROW, data));
			
			$(".info-edit").removeClass('hide');
			$(".inf-Blood").addClass('hide');
		...
		const do_bind_event_show_group = function(data){
			...
			$(".info-edit").on("click", function(){
				...
				$parent.find(".inf-Blood")	.removeClass("hide");

				$("#a_btn_sav, #a_btn_canc")	.removeClass("hide");

			})
			
//			$(".inf-Blood").on("click", function(){
//				let $parent = $(this).parent();
...
//			})
		...
		const do_lc_bind_event_new_row_table = function(data) {
		    $("#a_btn_sav, #a_btn_canc")	.removeClass("hide");
	        var newRow = tmplCtrl.req_lc_compile_tmpl(tmplName.TPY_CAT_Blood_ENT_CONTENT_ROW_ADD, {});
			
	        $('#tbody_Blood').append(newRow);
	        let addedRow = $('#tbody_Blood tr').last();
...
//	        do_init_autocomplete_member(prj);
		...
		const do_lc_after_get_Blood_sub = function(sharedJson, dataArray){
			if(can_gl_AjaxSuccess(sharedJson)) {
				let data 	= sharedJson[App['const'].RES_DATA];
				
//				$('.inf-Blood').addClass('hide');
				if (dataArray != null) {
					pr_Blood_TEMP = dataArray;
					$("#tbody_Blood").html(tmplCtrl.req_lc_compile_tmpl(tmplName.TPY_CAT_Blood_ENT_CONTENT_ROW, pr_Blood_TEMP));
					...
					$(".info-edit").removeClass("hide");
					$(".inf-Blood").addClass("hide");$("#btn_modify").removeClass("hide");
				} else {
					$("#tbody_Blood").html(tmplCtrl.req_lc_compile_tmpl(tmplName.TPY_CAT_Blood_ENT_CONTENT_ROW, data));
					$(".info-edit").addClass('hide');
				}
				...
				do_lc_bind_event_new_Blood_sub(data);
			} else {   
				do_gl_show_Notify_Msg_Error ($.i18n('common_err_msg_get'));
			}
		}
		
		const do_lc_bind_event_new_Blood_sub = function(data){
...
//				if(obj.hasError)	return false;
				...
				const rows = $('#tbody_Blood').find('tr');
				const dataArray = [];
				
				
				for (let i = 0; i < rows.length; i++) {
				    const row = rows[i];
				    const inputs = $(row).find('input.inf-Blood, select.inf-Blood');
				    const dataObject = {};
					...
					dataObject['stat'] = pr_STAT_Blood_ACTIVE;
					dataObject['typ01'] = pr_TYP_Blood_SUB;
...
//				myObject['parId'] = parentID;
				...
				do_lc_save_Blood_sub(myObject, dataArray);
			})
...
//				do_lc_get_Blood_sub(data);
		...
		const do_lc_save_Blood_sub = function(myObject, dataArray){
			const ref 				= req_gl_Request_Content_Send_With_Params(pr_SERVICE_CLASS, pr_SV_NEW_SUB, {obj: myObject});
			
			let fSucces		= [];
			fSucces.push(req_gl_funct(null, do_lc_after_get_Blood_sub, {dataArray}));
