define([
	'text!group/per/doctor/tmpl/PerDoctorGroup_List.html',
	'text!group/per/doctor/tmpl/PerDoctorGroup_List_Content.html',
	'text!group/per/doctor/tmpl/PerDoctorGroup_Ent_Content.html',
	'text!group/per/doctor/tmpl/PerDoctorGroup_New.html',
	'text!group/per/doctor/tmpl/PrjDropzone_File.html'
	
	],
	function(PerDoctorGroup_List, 
			PerDoctorGroup_List_Content,
			PerDoctorGroup_Ent_Content,
			PerDoctorGroup_New,
			PrjDropzone_File
			) {
	const PerDoctorGroupList = function (grpName, header, content, footer) {
		...
		const pr_SERVICE_CLASS_GROUP_DYN	= "ServicePerDoctor";
		const pr_SV_GROUP_LIST_DYN			= "SVLstDoctor"; 
		...
		this.do_lc_init		= function(){
			pr_ctr_Main 			= App.controller.PerDoctorGroup.Main;
			pr_ctr_List 			= App.controller.PerDoctorGroup.List
			if(!tmplName) {
				App.template.names[pr_grpName] = {}
				tmplName = App.template.names[pr_grpName]
			}
			
			tmplName.PRJ_USER_GROUP_LIST				= "PerDoctorGroup_List";
			tmplName.PRJ_USER_GROUP_LIST_CONTENT		= "PerDoctorGroup_List_Content";
			tmplName.PRJ_USER_GROUP_ENT_CONTENT		    = "PerDoctorGroup_Ent_Content";
			tmplName.PRJ_USER_GROUP_NEW  				= "PerDoctorGroup_New";
		...
		const do_lc_load_view = function(){
			tmplCtrl.do_lc_put_tmpl(tmplName.PRJ_USER_GROUP_LIST			, PerDoctorGroup_List); 
			tmplCtrl.do_lc_put_tmpl(tmplName.PRJ_USER_GROUP_LIST_CONTENT	, PerDoctorGroup_List_Content); 	
			tmplCtrl.do_lc_put_tmpl(tmplName.PRJ_USER_GROUP_ENT_CONTENT		, PerDoctorGroup_Ent_Content); 	
			tmplCtrl.do_lc_put_tmpl(tmplName.PRJ_USER_GROUP_NEW				, PerDoctorGroup_New); 	
		...
		const do_lc_get_list_member = (group) => {
			App.controller.PerDoctorGroup.Member.do_lc_show(group);
		};
...
//			if(data.hasError)	return false;
		...
		const do_lc_get_info_group = (group) => {
			...
			const ref 		= req_gl_Request_Content_Send_With_Params(pr_SERVICE_CLASS_GROUP_DYN, pr_SV_GET, {id: group.id});	
		...
		const do_lc_update_chat_group = function(ent) {
			const ref 		= req_gl_Request_Content_Send_With_Params(pr_SERVICE_CLASS_GROUP_DYN, pr_SV_MOD_GROUP, {obj: JSON.stringify(ent)});	
		...
		const do_lc_update_group_success = function(sharedJson){
			if(can_gl_AjaxSuccess(sharedJson)) {
				const data = sharedJson[App['const'].RES_DATA];
				if(data){
