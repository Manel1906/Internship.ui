define([
        'jquery',
        'text!group/per/producer/tmpl/Ent_Tabs.html',

		'group/per/producer/ctrl/EntTabInfo' ,
		'group/per/producer/ctrl/EntTabDoc' 
        ],
        function($, 
        		Tmpl_Ent_Tabs,      
...
//				do_gl_calculateScrollBody(pr_divContent + " .custom-scroll-tab", 43.6);
				...
				pr_ctr_EntTab_Info	.do_lc_show(pr_obj, pr_mode);
				pr_ctr_EntTab_Doc	.do_lc_show(pr_obj, pr_mode);
				
				do_bind_event(obj, mode);
			}catch(e) {			
				console.log(e);
				do_gl_exception_send(App.path.BASE_URL_API_PRIV,  "per.producer", "EntTabs", "do_lc_show", e.toString()) ;
			}
		};
