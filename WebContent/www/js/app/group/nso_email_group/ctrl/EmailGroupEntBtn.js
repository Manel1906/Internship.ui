define([
        'jquery',
        'text!group/nso_email_group/tmpl/EmailGroup_Ent_Btn.html'
	...
	var EmailGroupEntBtn     = function (grpName, header, content, footer) {
		...
		//-----------------------------------------------------------------------------------
		var pr_btn_Create 			= "#btn_PrjEmailGroup_create";
		var pr_btn_Edit 			= "#btn_PrjEmailGroup_edit";
		var pr_btn_Duplicate 		= "#btn_PrjEmailGroup_duplicate";
		var pr_btn_Del 				= "#btn_PrjEmailGroup_del";
		var pr_btn_Export 			= "#btn_PrjEmailGroup_export";
		var pr_btn_Send 			= "#btn_PrjEmailGroup_send";
		var pr_btn_Print 			= "#btn_PrjEmailGroup_print";
		var pr_btn_Save 			= "#btn_PrjEmailGroup_save";
		var pr_btn_Cancel 			= "#btn_PrjEmailGroup_cancel";
		var pr_btn_Transform 		= "#btn_PrjEmailGroup_transform";
		...
		this.do_lc_init		= function(){
			...
			pr_ctr_Rights			= App.controller.PrjEmailGroup.Rights;
		}
		...
		this.do_lc_show		= function(obj, mode){
			...
			pr_mode		= mode;
			try{
				tmplCtrl.do_lc_put_tmpl(tmplName.EMAIL_GROUP_ENT_BTN	, EmailGroup_Ent_Btn); 			
				$(pr_divContent).html(tmplCtrl.req_lc_compile_tmpl(tmplName.EMAIL_GROUP_ENT_BTN, pr_object));
				...
				console.log(e); //do_gl_send_exception(App.path.BASE_URL_API_PRIV, this.var_lc_URL_Header, App.network, "tpy.cat", "EmailGroupEntBtn", "do_lc_show", e.toString()) ;
			}
		};
		...
		var do_bind_event_btn_transform		= function(obj, mode){
			...
			$(pr_btn_Transform	).click(function(){
				tmplCtrl.do_lc_put_tmpl(tmplName.TPYCAT_POPUP_DUPLICATE			, EmailGroup_Popup_Duplicate);
				tmplCtrl.do_lc_put_tmpl(tmplName.TPYCAT_POPUP_DUPLICATE_SELECT_SOC	, EmailGroup_Popup_Duplicate_Select_Soc);
		...
		function do_bind_event_popup_dupliacte(){
			if(App.data["LstSocieteChild"]){
				...
				$.each(App.data["catsList"], function (key, val){
					var div = "#div_PrjEmailGroup_Popup_Duplicate_" + key + " input[type='checkbox']";
