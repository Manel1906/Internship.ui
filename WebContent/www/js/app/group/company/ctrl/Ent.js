define([
	...
	var Ent 	= function (grpName, header, content, footer) {
		...
		var pr_divContent 			= "#div_company_content";
		var pr_divFooter 			= footer  ? footer : null;

		const pr_divTabDocs			= "#div_prj_docs";
		const pr_divTabPerInfo 		= "#div_company_info_person";
		...
		//------------------const object------------------------------------------------------
		const typePrjMngCompany	= 1010002;
...
//			do_get_pos_position();
		...
		const do_lc_get_Entity = function(id, mode){
			console.log("new company do_lc_get_Entity ");
...
//				self.do_lc_Save_Entity("#div_user_ent", obj, var_lc_MODE_NEW); 
		...
		const do_get_per_societe = function(typeSoc) {
			//ajax to get all fix values here
			var ref 		= req_gl_Request_Content_Send('ServicePerPerson', 'SVGet');
			ref["typ02"]	= typeSoc;
