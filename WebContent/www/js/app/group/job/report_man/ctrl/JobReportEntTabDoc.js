define(['jquery'], function($) {
	...
	var JobReportEntTabDoc     = function (grpName, header, content, footer) {
		...
		this.do_lc_show		= function(obj, mode){
			try{
				$(pr_divContent).html(tmplCtrl.req_lc_compile_tmpl(tmplName.JOB_REPORT_ENT_TAB_DOC, obj));//---add all files to template, then delete them in dropzone
		...
		var do_lc_bind_event_docs = function(obj, mode){
			if(obj.stat != pr_stat_draft && obj.stat != pr_stat_denied){
				$("#btn_add_doc").hide();
			}else{
				$("#btn_add_doc").show();
			}
