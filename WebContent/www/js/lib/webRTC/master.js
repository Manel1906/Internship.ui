...
async function startMaster(localView, remoteView, formValues, onStatsReport, onRemoteDataMessage, onActionReady, onClientResponse) {
    console.log("config ", formValues);
        ...
        peerConnection.addEventListener('track', event => {
            ...
            for(let i = 0; i < remoteView.length; i++){
            	...
            	if (remoteViewItem.srcObject) {
                    if (remoteViewItem.dataset.client == remoteClientId) {
                        remoteViewItem.srcObject = event.streams[0];
                        break;
                    }
            		continue;
//                    return;
            	...
            	remoteViewItem.srcObject = event.streams[0];
            	$(remoteViewItem).attr("data-client", remoteClientId);
                break;
            }
...
//        master.remoteView.srcObject = null;
        for(let i = 0; i < master.remoteView.length; i++){
        	const remoteViewItem = master.remoteView[i];
